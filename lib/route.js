...
class Route {
  ...
  resource(name, ...args) {
    ...
    if (!controllerPath) {
      throw new Error(`controller is not specify: ${JSON.stringify(last)}`);
    }
...
const controllerDebug = debug.extend('base_controller');
const controllerParamsDebug = controllerDebug.extend('params');
  ...
  static getProcess(actionName, controllerPath) {
    ...
    const actionCaller = {
      [fullName]: async (req, res, _next) => {
        controllerDebug(`${req.method} ${req.path} ${fullName}`);
        if(controllerParamsDebug.enabled) {
          controllerParamsDebug(`req.params: %o`, req.params);
          controllerParamsDebug(`req.body: %o`, req.body);
          controllerParamsDebug(`req.query: %o`, req.query);
        }
