...
const matchParamsDebug = matchDebug.extend('params');
    ...
    const routerParams = middlewares.map((middleware) => {
      if (typeof middleware === 'string') {
        ...
        return async (req, res, next) => {
          ...
          if(matchParamsDebug.enabled) {
            matchParamsDebug(`req.params: %o`, req.params);
            matchParamsDebug(`req.body: %o`, req.body);
            matchParamsDebug(`req.query: %o`, req.query);
          }
          try {
            return await this.getActionProcess(controllerPath, actionName)(req, res);
          } catch(err) {
            matchDebug(err.stack);
            next(err, req, res);
          }
        };
      } else {
